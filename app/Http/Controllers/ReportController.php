...
class ReportController extends Controller
{
    public function download_report($quarter_id)
    {
        ...
        $staff = Staff::where('user_id', $user->userId)->first();
    ...
    public function show_report(Request $request)
    {
        ...
        if ($user->classification_name == 'tmt' || $user->classification_name == 'exco') {
            $performance_Score = number_format((($competence_score / ($total_weights)) * 60), 2);
        } else {
            $performance_Score = number_format((($competence_score / ($total_weights)) * 70), 2); // Adjust as per your score scaling
        }

        if ($user->classification_name == 'tmt' || $user->classification_name == 'exco') {
            $culture_score = number_format(((($culture->equity + $culture->excellence + $culture->integrity + $culture->teamwork + $culture->people) / 30) * 40), 2);
        } else {
            $culture_score = number_format(($culture->equity + $culture->excellence + $culture->integrity + $culture->teamwork + $culture->people), 2); // Adjust as per your score scaling
        }
        // $culture_score = $culture->equity + $culture->excellence + $culture->integrity + $culture->teamwork + $culture->people;
    ...
    public function getSVGForResult($avgscore, $avgCulture, $avgPerformance)
    {
        ...
        if ($avgPerformance >= 55) {
            $performanceSvg = 'high'; // Display high score SVG
        } elseif ($avgPerformance >= 45) {
    ...
    public function show_departmental_report(Request $request)
    {
        ...
        $users = User::where('department_id', $departmentid)
            ->with('job')->orderBy("firstName", "asc")
            ->get();
        ...
        foreach ($users as $user) {
            ...
            if ($cultureData != []) {
                if ($user->classification_name == 'tmt' || $user->classification_name == 'exco') {
                    $cultureScore = ($cultureData->equity + $cultureData->excellence + $cultureData->integrity + $cultureData->teamwork + $cultureData->people) / 30 * 40;
                } else {
                    $cultureScore = $cultureData->equity + $cultureData->excellence + $cultureData->integrity + $cultureData->teamwork + $cultureData->people; // Adjust as per your score scaling
                }
            } else {
                $cultureScore = 0;
            }
            ...
            $totalTasksScore = 0;
            $tasks = Task::where('quarter_id', $quarter->id)->where('user_id', $user->userId)->get();
            // $user->tasks; // Assuming User has a relation 'tasks'
            ...
            if ($totalTasksScore > 1) {
                if ($user->classification_name == 'tmt' || $user->classification_name == 'exco') {
                    $userScoreOutOf70 = ($totalUserScore / ($totalTasksScore)) * 60;
                } else {
    ...
    public function show_overall_report(Request $request)
    {
        $validated = $request->validate([
            'quarter_id' => "required",
            // 'dept_id' => "required",
        ...
        // $department = Department::find($departmentid);
        // $users = $department->users; // Assuming Department has a relation 'users'
        $users = User::with('job')->orderBy("firstName", "asc")
            ->get();
    ...
    public function show_management_report(Request $request)
    {
        $validated = $request->validate([
            'quarter_id' => "required",
            // 'dept_id' => "required",
        ...
        // $department = Department::find($departmentid);
        // $users = $department->users; // Assuming Department has a relation 'users'
        $users = User::whereIn('classification_name', ['tmt', 'exco'])->with('job')->orderBy("firstName", "asc")
            ->get();
        // where('department_id', $departmentid)
